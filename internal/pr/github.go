...
func (r *GitHubResolver) Resolve(ctx context.Context, info *PRInfo) (*PRMetadata, error) {
	...
	args := []string{
		...
		"--json", "baseRefName,headRefName,headRefOid,state,title,mergedAt",
	}
	...
	var ghResp struct {
		...
		MergedAt    *string `json:"mergedAt"` // nil if not merged, timestamp string if merged
	}
	...
	// PR is merged if mergedAt has a value
	isMerged := ghResp.MergedAt != nil && *ghResp.MergedAt != ""
	...
	return &PRMetadata{
		...
		State:    normalizeState(ghResp.State, isMerged),
		IsMerged: isMerged,
	}, nil
}
